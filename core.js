...
function output(t){const code=t.format.code;const groups=new Map();for(const r of assess(t).filter(r=>r.status==='採用')){const category=t.format.category&&t.categoryCol>=0?(r.cells[t.categoryCol]||'未分類'):'';if(!groups.has(category))groups.set(category,[]);groups.get(category).push(r);}
 ...
 const blocks=rows.map((r,i)=>{const title=safe(r.cells[t.nameCol],code);const lines=[`${t.format.number?`${i+1}. `:''}${title}`];for(const c of t.fields){if(c===t.nameCol||!r.cells[c])continue;lines.push(`${t.format.labels?safe(t.headers[c],code)+'：':''}${safe(r.cells[c],code)}`);}return lines.join('\n');});
