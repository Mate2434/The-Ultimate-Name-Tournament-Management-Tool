...
function output(t){const code=t.format.code;const groups=new Map();for(const r of assess(t).filter(r=>r.status==='採用')){const category=!Array.isArray(t.extraCols)&&t.format.category&&t.categoryCol>=0?(r.cells[t.categoryCol]||'未分類'):'';if(!groups.has(category))groups.set(category,[]);groups.get(category).push(r);}
 ...
 for(const b of blocks){if(wrap(b).length>1900){emit();const room=1900-wrap('').length;if(room<50)throw Error('見出しが長すぎます。');let chars=Array.from(b);while(chars.length){let piece='';while(chars.length&&piece.length+chars[0].length<=room)piece+=chars.shift();push(wrap(piece));}continue;}if(code){emit();push(wrap(b));continue;}if(wrap(body+(body?'\n\n':'')+b).length>1900)emit();body+=(body?'\n\n':'')+b;}emit();}
