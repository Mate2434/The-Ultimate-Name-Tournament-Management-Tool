...
function assess(t){const counts=new Map();return t.rows.map((cells,i)=>{const id=normalizeID(cells[t.idCol]);const hasName=String(cells[t.nameCol]||'').trim();let status='受付済み',n=0;if(!id)status='ID未入力';else if(!hasName)status='名前未入力';else{n=(counts.get(id)||0)+1;counts.set(id,n);if(t.limit>0&&n>t.limit)status='上限超過';}return {cells,id,n,status,row:i+2};});}
function safe(x,code=false){let s=String(x??'').replace(/@/g,'@\u200b').replace(/```/g,'``\u200b`');return code?s:s.replace(/([\\*_~`|>#[\]])/g,'\\$1');}
function output(t){const code=t.format.code;const groups=new Map();for(const r of assess(t).filter(r=>r.status==='受付済み')){const category=!Array.isArray(t.extraCols)&&t.format.category&&t.categoryCol>=0?(r.cells[t.categoryCol]||'未分類'):'';if(!groups.has(category))groups.set(category,[]);groups.get(category).push(r);}
 const messages=[];
 ...
 const wrap=b=>(heading?heading+'\n':'')+(code?'```\n'+b+'\n```':b);
 for(const b of blocks){
  if(wrap(b).length<=1900){messages.push(wrap(b));continue;}
  const room=1900-wrap('').length;if(room<50)throw Error('見出しが長すぎます。');
  let piece='';for(const char of b){if(piece.length+char.length>room){messages.push(wrap(piece));piece='';}piece+=char;}if(piece)messages.push(wrap(piece));
 }
 }
 return messages;
}
